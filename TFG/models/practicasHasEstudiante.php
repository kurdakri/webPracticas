...
class PracticasHasEstudiante{
	...
	public function selectPri($id,$pri){
		$this->con = $this->db->getConnection();
		$sql = 'select * from practicas_has_estudiante where Estudiante_idEstudiante="' . $id . '" and Prioridad="'.$pri.'"';
        $result = mysql_query($sql, $this->con);
        if (mysql_num_rows($result) == 0) {
			$this->db->endConnection();
            return false;
	...
	public function deleteEstudiante($id){
		$this->con = $this->db->getConnection();
		$sql = 'delete from practicas_has_estudiante where Estudiante_idEstudiante="'.$id.'"';
	    $result = mysql_query($sql,$this->con);
	    if($result == true){
			$this->db->endConnection();	
			return true;
